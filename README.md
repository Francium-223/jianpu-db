...
tag=这首歌的标签1，这首歌的标签2（用逗号隔开）
或者不带前缀用回车分隔
alias=这首歌的别名1，这首歌的别名2（用逗号隔开）（如果你想写的话）
alias=或者用回车分隔（如果你想写的话）
...
以[th01_01.txt](scores/th01_01.txt)（A Sacred Lot）为例：
...
因为本数据集记录的是旋律，在[MusicBrainz](https://musicbrainz.org/)中的索引应以**work**为单位，除非某一改版有不同的旋律。
...
|键|值|含义|
|---|---|---|
|N/A|"310b3b07-ec9f-3e88-9fd8-529c17478179"|该work在[MusicBrainz](https://musicbrainz.org/)中唯一且确定的标识符（**MBID**）为`310b3b07-ec9f-3e88-9fd8-529c17478179`。|
|"file"|"th01_02.txt"|这首歌的**曲谱文件**为`scores/th01_02.txt`。注意所有曲谱都应是`scores`的一级子文件，且统一以“.txt”结尾。|
|"title"|"永遠の巫女"|这首歌的**标题**为`永遠の巫女`。此处应填写其最早发布版本的现时通用名称。|
|"tag"|\["th01", "东方灵异传", "东方", "东方原曲", "ZUN", "东方旧作原曲", "东方整数作原曲"\]|这首歌的**标签**有`th01`和`东方灵异传`、`东方`、`东方原曲`、`ZUN`、`东方旧作原曲`。"tag"对应的列表里的字符串用来给歌曲分门别类。根据这些字符串会自动在`by_tag`中生成快捷方式。|
|"alias"|\["永远之巫女"\]|这首歌的**别名**有：`永远之巫女`、`永远的巫女`，可以填写多个。包括但不仅限于：可选的其他拼写方式、转写、翻译、在社群中有一定影响力的别名。|
|"transcriber"|\["Francium-223"\]|该文件的转写者为[Francium-223](https://github.com/Francium-223/)。|

其他键在其各自文件夹中建立索引，如果有一个`"foo": ["bar"]`，索引会建立在一个叫`by_foo`的文件夹。**除`file`、`title`外，其他键对应的值均为列表。**

一般来说，一些标签具有蕴涵关系，可以只写一个标签也能自动生成（PS：脚本还没写好）对应的标签。（见[tag_imply.json](tag_imply.json)和[tag_equal.json](tag_equal.json)。）一般来说，如果有100首含有标签`foo`的歌，其中有99首歌都同时也有标签`bar`，有1首歌没有标签`bar`，那么应该在[tag_imply.json](tag_imply.json)里增加一项
