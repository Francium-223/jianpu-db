...
|键|值|含义|
|---|---|---|
|N/A|310b3b07-ec9f-3e88-9fd8-529c17478179|该work在[MusicBrainz](https://musicbrainz.org/)中唯一且确定的标识符为`310b3b07-ec9f-3e88-9fd8-529c17478179`。|
|"file"|"th01_02.txt"|这首歌的曲谱文件为`scores/th01_02.txt`。注意所有曲谱都应是`scores`的一级子文件，且统一以“.txt”结尾。|
|"title"|"永遠の巫女"|这首歌的标题为`永遠の巫女`。此处应填写其最早发布版本的现时通用名称。|
|"alias"|\["永远之巫女"\]|这首歌的别名有一个：`永远之巫女`，可以填写多个。包括但不仅限于：可选的其他拼写方式、转写、翻译、在社群中有一定影响力的别名。|
|"tag"|\["th01", "东方灵异传"\]|这首歌的标签有`th01`和`东方灵异传`。"tag"对应的列表里的字符串用来给歌曲分门别类。根据这些字符串会自动在`by_tag`中生成快捷方式。|

有的标签并不会在：`by_tag`中建立索引，目前这些标签包括：`transcriber:foo`、`instrument:foo`、`artist:foo`、`bpm:foo`，而是在一个叫`by_[:foo前的那一串字]`的文件夹。如添加一个`transcriber:foo`的标签会使其在`by_transcriber`文件夹中建立索引。

一般来说，一些标签具有蕴涵关系，可以只写一个标签也能自动生成（PS：脚本还没写好）对应的标签。（见[tag_imply.json](tag_imply.json)和[tag_equal.json](tag_equal.json)。）一般来说，如果有100首含有标签`foo`的歌，其中有99首歌都同时也有标签`bar`，有1首歌没有标签`bar`，那么应该在tag_imply.json里增加一项
